...
function previewFive() {
   //lacate first element where the tag is article assign to a variable
   var articleEI = document.getElementsByTagName("article")[0]; 
   
   //create fiigure and img elements for fifth image 
   ...
   var firstFigure = lastFigure.cloneNod(true);
   
   firstFigure.id = "fig 1"; 
   firstFigure.style.right = " "; 
   firstFigure.style.left = "45px";
   
   articleEI.insertBefore(firstFigure, document.getElementById("fig 2"));
...
function createEventListeners() {
   ...
   //declare variable for document element with id of leftarrrow, which is in the html document
   var leftarrow = document.getElementById("leftarrow"); 

    //add an event handler to the specified document element (leftarrow) 
    //the leftArrow() function will be executed when the user clicks the left navigation arrow buttone
   ...
   //when the user clicks the Show More button the previewFive fucion is executed
